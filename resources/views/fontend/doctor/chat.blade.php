...
<div class="col-lg-8 chat-cont-right">
        ...
        <div class="media d-flex">
            <div class="media-img-wrap flex-shrink-0">
                <div class="avatar avatar-online">
                 ...
                 <img src="{{ $booking->patient->image() }}" alt="User Image" class="avatar-img rounded-circle" />
                 @else
                 <img src="{{ asset('contents/fontend') }}/assets/img/doctors/doctor-thumb-02.jpg" alt="User Image" class="avatar-img rounded-circle" />
                 @endif
                </div>
            </div>
            <div class="media-body flex-grow-1">
                <div class="user-name">{{ $booking->patient->name }}</div>
    ...
    <div class="chat-footer">
        <form id="chat-form" action="{{ route('website.doctor.chat.submit',$booking->id) }}" method="post">
            @csrf
...
<script>
  ...
  $("#chat-form").on("submit", function(e){
   ...
   function chat_ajax(){
        var url='{{ route('website.doctor.chat.ajax',$booking->id) }}';
        $.ajax({
          url:url,
          type:'GET',
          dataType:'html',
